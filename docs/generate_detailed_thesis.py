#!/usr/bin/env python3
# filepath: /home/sashankhravi/Documents/stock_rl_agent_nifty_50/docs/generate_detailed_thesis.py
...
def generate_enriched_thesis_pdf():
    """
    Generate a complete thesis PDF with detailed indicator explanations
    """
    ...
    # Define paths
    enriched_md_path = current_dir / 'nifty500_trading_system_thesis_enriched.md'
    output_pdf = current_dir / 'Nifty500_Trading_System_Detailed.pdf'
    image_dir = current_dir / 'images'
    
    # First, make sure we have the enriched markdown file
    if not enriched_md_path.exists():
        print("Enriched thesis markdown not found. Running merger script first...")
        
        # Run the merge script to create the enriched markdown
        merge_script = current_dir / 'merge_indicator_explanations.py'
        if not merge_script.exists():
            print(f"Error: Merge script not found at {merge_script}")
            return False
        
        result = subprocess.run(['python', str(merge_script)], 
                               cwd=current_dir, 
                               capture_output=True, 
                               text=True)
    ...
    # Read the enriched markdown file
    print(f"Reading enriched markdown content from: {enriched_md_path}")
    try:
        with open(enriched_md_path, 'r', encoding='utf-8') as f:
            md_content = f.read()
    except Exception as e:
        print(f"Error reading enriched markdown file: {e}")
        return False
    ...
    temp_html_path = current_dir / 'detailed_thesis_temp.html'
