#!/usr/bin/env python3
# filepath: /home/sashankhravi/Documents/stock_rl_agent_nifty_50/docs/pdf_validator.py
import os
import sys
...
def validate_pdf():
    ...
    # Check the PDF version date
    from datetime import datetime
    current_date = datetime.now().strftime("%B %d, %Y")
    print(f"\n✓ PDF generation date: {current_date}")
    ...
    try:
        ...
        with open(pdf_path, "rb") as f:
            ...
            print("\nPDF Metadata:")
            print(f"  - Title: {info.title if hasattr(info, 'title') else 'Not set'}")
            ...
            if has_images:
                print("  - ✓ PDF contains embedded images")
            else:
                print("  - WARNING: No embedded images detected in the PDF")
                
    except ImportError:
        print("\nNOTE: PyPDF2 not installed. Skipping detailed PDF metadata check.")
    except Exception as e:
        print(f"\nError checking PDF metadata: {e}")
